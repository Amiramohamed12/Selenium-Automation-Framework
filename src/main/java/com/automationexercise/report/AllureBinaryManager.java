...
public class AllureBinaryManager {
    ...
    {
        static final String VERSION = resolveVersion(); //2.42.1
    ...
    public static void downloadAndExtract() {
        try {
            ...
            //c:\Users\lenovo\.m2\repository\allure\allure-2.42.1
            ...
            if (!OSUtils.getCurrentOS().equals(OSUtils.OS.WINDOWS)) {
                 TermnialUtils.executeTerminalCommand("chmod", "u+x", AllureConstants.USER_DIR.toString());
            }
            ...
            // Give execute permissions to the binary if not on Windows
            if (!OSUtils.getCurrentOS().equals(OSUtils.OS.WINDOWS)) {
               TermnialUtils.executeTerminalCommand("chmod", "u+x", getExecutable().toString());
            }
    ...
    public static Path getExecutable() {
        String version = LazyHolder.VERSION;
        //C:\Users\lenovo\.m2\repository\allure\allure-2.34.1\bin
